...
{
    std::string link;
    int result = yfs->readlink(ino, link);

    if (result ==  yfs_client::OK) {
        if (fuse_reply_readlink(req, link.c_str())) {
            fuse_reply_err(req, ENOENT);
        }   
    } else {
        // if error, need to return error code but I couldn't find any reference in fuse_lowlevel.h
        // therefore I refer the error code tas in fuseserver_mkdir and fuseserver_create
        if (result == yfs_client::EXIST) {
            fuse_reply_err(req, EEXIST);
        } else {
            fuse_reply_err(req, ENOENT);
        }
...
fuseserver_symlink(fuse_req_t req, const char *link, fuse_ino_t parent, const char *name)
{
    ...
    yfs_client::inum tempInum = 0; // 0 is added to avoid compiler warning. Will be changed later in yfs->symlink()
    int stat = yfs->symlink(parent, name, link, tempInum);
